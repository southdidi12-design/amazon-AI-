...
class AmazonAdBot:
    ...
    def optimize_bids(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if cond_loser.any():
            ...
            # 应用下限
            df.loc[cond_loser, 'new_bid'] = np.maximum(target_cpc_loser, self.config.MIN_BID)
    ...
    def run(self):
        ...
        # 5. 进入交互式操作模式
        self.interactive_apply(optimized_data)
    ...
    def export_report(self, df):
        ...
        display_df['acos'] = display_df['acos'].apply(lambda x: f"{x:.1%}" if x != float('inf') else "Inf")
    ...
    def interactive_apply(self, df: pd.DataFrame):
        """交互式地让用户确认并模拟应用更改"""
        print("\n" + "="*80)
        print("交互式操作模块")
        print("="*80)
        
        changes_df = df[df['action'] != self.ACTION_HOLD].copy()
        if changes_df.empty:
            logger.info("所有关键词均无需调整，未进入操作模式。")
            return

        print("AI 助手已生成以下建议。请逐一确认：")
        ...
        for _, row in changes_df.iterrows():
            prompt = (
                f"\n关键词: '{row['keyword']}'\n"
                f"  - 建议操作: {row['action']}\n"
                f"  - 建议出价: ${row['new_bid']:.2f}\n"
                f"是否批准此项操作? (y/n/q 批准/跳过/退出): "
            )
            choice = input(prompt).lower()

            if choice == 'y':
                logger.info(f"已批准: '{row['keyword']}' 的出价将调整为 ${row['new_bid']:.2f} (模拟)。")
                # 在这里，未来会调用真正的 Amazon API 函数
            elif choice == 'q':
                logger.info("用户退出操作。")
                break
            else:
                logger.info(f"已跳过: '{row['keyword']}'。")
        
        logger.info("交互式应用会话结束。")
